...
// // Razorpay Configuration
// const RAZORPAY_KEY_ID = import.meta.env.VITE_APP_RAZORPAY_KEY_ID || 'rzp_test_R6mBF5iIMakFt1';
// const BACKEND_BASE_URL = import.meta.env.VITE_APP_API_URL || 'http://localhost:5000/api';
...
// const BACKEND_BASE_URL = import.meta.env.VITE_APP_API_URL || 'http://localhost:5000/api';
// const RAZORPAY_KEY_ID = import.meta.env.VITE_APP_RAZORPAY_KEY_ID || 'rzp_live_RCcUEL1jJAbFmy';
...
//         const filteredPlans = response.data.filter(plan => {
//           const matchesType =
//             (planType === 'team' && plan.type === 'business') ||
//             (planType === 'individual' && plan.type === 'individual');
//           const planInterval = plan.interval?.toLowerCase();
//           const matchesInterval =
//             (billingCycle === 'monthly' && ['month', 'monthly'].includes(planInterval)) ||
//             (billingCycle === 'yearly' && ['year', 'yearly', 'annual'].includes(planInterval)) ||
//             (billingCycle === 'quarterly' && ['quarter', 'quarterly'].includes(planInterval));
//           return matchesType && matchesInterval;
//         });
...
//   const handleSelectPlan = async plan => {
//     if (!plan.id || !plan.price) return setError('Invalid plan selected');
...
//       // Call backend to start subscription
//       const response = await fetch(`${BACKEND_BASE_URL}/payments/subscription/start`, {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
...
//         handler: async function (response) {
//           try {
//             const verify = await fetch(`${BACKEND_BASE_URL}/payments/subscription/verify`, {
//               method: 'POST',
//               headers: {
//                 'Content-Type': 'application/json',
//                 Authorization: `Bearer ${token}`,
...
//             if (verifyResult.success) {
//               handlePaymentSuccess(plan.name, response);
//             } else {
//               throw new Error(verifyResult.message || 'Payment verification failed');
//             }
//           } catch (err) {
//             handlePaymentFailure({ description: err.message });
//           } finally {
//             setProcessingPayment(false);
//             setSelectedPlanId(null);
//           }
...
//         {/* Billing and Plan Type toggles here (omitted for brevity) */}
...
//             <div className="flex justify-between items-center">
//               <div className="text-red-800">{error}</div>
//               <button onClick={fetchPlans} className="text-red-600 hover:text-red-500 text-sm font-medium">
//                 Retry
//               </button>
//             </div>
//           </div>
...
//         <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
//           {plans.map(plan => (
//             <div key={plan.id} className="bg-white rounded-lg shadow-lg p-8 flex flex-col border">
//               <h2 className="text-2xl font-bold text-gray-900 mb-2">{plan.name}</h2>
//               <p className="text-gray-500 text-sm mb-6">{plan.description}</p>
//               <div className="flex items-baseline mb-6">
//                 <span className="text-4xl font-extrabold text-gray-900">₹{plan.price}</span>
//                 <span className="ml-1 text-gray-500 text-base">/{billingCycle}</span>
//               </div>
//               <button
//                 onClick={() => handleSelectPlan(plan)}
//                 disabled={processingPayment}
//                 className="w-full py-3 px-6 rounded-md text-base font-medium bg-gray-900 text-white hover:bg-gray-800"
//               >
//                 {processingPayment && selectedPlanId === plan.id ? 'Processing...' : 'Select Plan'}
//               </button>
...
//       // Start subscription on backend (live)
//       const response = await fetch(`${BACKEND_BASE_URL}/payments/subscription/start`, {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
...
//       const data = await response.json();
//       if (!data.success || !data.subscription?.id || !data.subscription?.key) {
//         throw new Error(data.message || 'Failed to start subscription');
//       }

//       // Razorpay live key comes from backend
//       const options = {
//         key: data.subscription.key, // <-- LIVE key from backend
//         subscription_id: data.subscription.id,
//         name: 'NexintelAI Subscriptions',
//         description: `${plan.name} Subscription`,
//         image: 'https://www.nexintelai.com/assets/img/Ai%20logo-01.png',
...
const SubscriptionPlanPage = () => {
  ...
  const handlePaymentFailure = error => {
    const msg = error?.description || error?.message || 'Payment failed';
    console.error('Payment failed:', error); // Add logging
    alert(`❌ Payment Failed: ${msg}`);
    setProcessingPayment(false);
    setSelectedPlanId(null);
  };
  ...
  const handleSelectPlan = async plan => {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text();
        console.error('Server response:', text); // Add logging
        throw new Error(`Server error: ${response.status} ${text}`);
      }

      const data = await response.json();
      console.log('Subscription response:', data); // Add logging
      
      if (!data.success || !data.subscription?.id || !data.subscription?.key) {
        throw new Error(data.message || 'Failed to start subscription - missing required data');
      }

      // Initialize Razorpay with live key from backend
      const options = {
        key: data.subscription.key, // Live key from backend
        subscription_id: data.subscription.id,
        name: 'NexintelAI Subscriptions',
        ...
        handler: async response => {
          ...
          try {
            ...
            const verifyResult = await verify.json();
            console.log('Verification result:', verifyResult); // Add logging
        ...
        modal: {
          ondismiss: function () {
            console.log('Payment modal dismissed'); // Add logging
            setProcessingPayment(false);
            setSelectedPlanId(null);
          },
          escape: false, // Prevent ESC key from closing modal
          backdropclose: false // Prevent backdrop click from closing modal
        },
        notes: {
          plan_id: plan.id,
          user_id: user?.id || 'anonymous',
      ...
      console.log('Razorpay options:', options); // Add logging
      const rzp = new window.Razorpay(options);
      
      rzp.on('payment.failed', function (response) {
        console.error('Payment failed event:', response); // Add logging
        handlePaymentFailure(response.error);
      });
      
      rzp.open();
    } catch (err) {
      console.error('Error in handleSelectPlan:', err); // Add logging
      setError(err.message || 'Payment process failed');
      setProcessingPayment(false);
      setSelectedPlanId(null);
    }
  ...
  // Add UI for billing cycle and plan type selection
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <button
          ...
          {plans.map(plan => (
            <div key={plan.id} className="bg-white rounded-lg shadow-lg p-8 flex flex-col border">
              ...
              <button
                onClick={() => handleSelectPlan(plan)}
                disabled={processingPayment}
                className={`w-full py-3 px-6 rounded-md text-base font-medium transition-colors ${
        ...
        {!loading && plans.length === 0 && !error && (
          <p className="text-center text-gray-600">
            No plans available for the selected billing cycle and type.
          </p>
        )}
      </div>
    </div>
